...
const scaleFactorIncrease = 0.0001; // Exponential increase in scale factor over time
...
function animate(currentTime) {
    ...
    tori.forEach(({ torus }) => {
        if (torus.added) {
			...
			// Check if torus position has looped back to origin
			if (z >= initialZ) {
				torus.scale.set(1, 1, 1); // Reset scale to 1
			} else {
